...
\033[0;32m               
      ...
      ---|Có thể tải tool tại website www.mecloud.vn|----   
    ======================================================             
              Copyright © 2020 By MECLOUD.VN           
\n";
//config
{
@system('clear');
...
if ($source != 1 && $source != ''){
	...
	if (trim(fgets(STDIN)) == 'y'){
		...
		echo $white."Delay Like (Min 50 Giây): $green";
		$_SESSION['delaylike']=trim(fgets(STDIN));
		if($_SESSION['delaylike'] < 50)
		{exit($red."Tối Thiểu 50 Giây\n");}
	}
	echo $white."Nhiệm vụ sub (y/n): $green";
	if (trim(fgets(STDIN)) == 'y'){
		array_push($listnv,'sub');
		echo $white."Delay Sub (Min 50 Giây): $green";
		$_SESSION['delaysub']=trim(fgets(STDIN));
		if($_SESSION['delaysub'] < 50)
		{exit($red."Tối Thiểu 50 Giây\n");}
	}
	echo $white."Nhiệm vụ cmt (y/n): $green";
	if (trim(fgets(STDIN)) == 'y'){
		array_push($listnv,'cmt');
		echo $white."Delay CMT (Min 50 Giây): $green";
		$_SESSION['delaycmt']=trim(fgets(STDIN));
		if($_SESSION['delaycmt'] < 50)
		{exit($red."Tối Thiểu 10 Giây\n");}
	}
	echo $white."Nhiệm vụ page (y/n): $green";
	if (trim(fgets(STDIN)) == 'y'){
		array_push($listnv,'page');
		echo $white."Delay Page (Min 50 Giây): $green";
		$_SESSION['delaypage']=trim(fgets(STDIN));
		if($_SESSION['delaypage'] < 50)
		{exit($red."Tối Thiểu 50 Giây\n");}
	}
	echo $white."Nhiệm vụ cảm xúc (y/n): $green";
	if (trim(fgets(STDIN)) == 'y'){
		array_push($listnv,'cx');
		echo $white."Delay cảm xúc (Min 50 Giây): $green";
		$_SESSION['delaycx']=trim(fgets(STDIN));
		if($_SESSION['delaycx'] < 50)
		{exit($red."Tối Thiểu 50 Giây\n");}
	}
		...
		while ($i <= $_SESSION['i']){
			...
			if ($rand == 'like'){
				...
				foreach ($list  as $id) {
					...
					$s = nhantien('like',$id);
					if ($s == 2){$xu = $xu + 300; echo $white."[LIKE]|$green ID:".$id." -> $white Thành Công$green +300 xu -> ".$white.$xu;}
				...
				foreach ($list  as $id) {
					...
					$s = nhantien('sub',$id);
					if ($s == 2){$xu = $xu + 600;echo $white."[FOLLOW]|$green ID:".$id." -> $white Thành Công$green +600 xu -> ".$white.$xu;}
				...
				foreach ($list  as $id) {
					page($id,$cookie);
					$s = nhantien('page',$id);
					if ($s == 2){$xu = $xu + 600; echo $white."[PAGE]|$green ID:".$id." -> $white Thành Công$green +600 xu -> ".$white.$xu;}
				...
				foreach ($list  as $id => $key) {
					...
					if ($s == 2){$xu = $xu + 400; echo $white."[$type]|$green ID:".$id." -> $white Thành Công$green +400 xu -> ".$white.$xu;}
